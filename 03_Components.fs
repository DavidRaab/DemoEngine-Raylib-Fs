...
module Camera =
    ...
    let calculateMatrix camera : Matrix4x4 =
    ...
    let matrix (camera:Camera) : Matrix4x4 =
    ...
    let screenToWorld (position:Vector2) (camera:Camera) : Vector2 =
        match Matrix.Invert (matrix camera) with
        | true, matrix -> Vector2.Transform(position, matrix)
